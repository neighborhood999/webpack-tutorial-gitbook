...
所以現在我們寫了一個設定檔讓我們可以 minify 和 bundle 我們的 JavaScript。這個 bundle 檔案可以被複製並貼到其他的專案目錄中，放入 `<script>` 就可以使用。如果你只需要了解怎麼用 webpack 處理*只有 JavaScript* 的基本情況，你可以直接跳到[結論](#conclusion)。
